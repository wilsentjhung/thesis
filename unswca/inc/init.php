...
$i = 0;
$courses = array();
$query = "SELECT p.course_code AS code, p.title AS title, p.career AS career, p.uoc AS uoc, p.norm_pre_req_conditions AS prereq, c.norm_co_req_conditions AS coreq, q.norm_equivalence_conditions AS equiv, x.norm_exclusion_conditions AS excl, s.area AS subject_area
          ...
          JOIN subject_area s ON x.course_code = s.course_code AND x.career = s.career";
$result = pg_query($aims_db_connection, $query);
while ($rows = pg_fetch_array($result)) {
    $course = new Course($rows["code"], $rows["title"], $rows["career"], $rows["uoc"], $rows["prereq"], $rows["coreq"], $rows["equiv"], $rows["excl"], $rows["subject_area"]);
